    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        try {
          ...
          console.log(
            `  Current chain: ${currentChainId}, Required: ${CONFIG.CHAIN_ID}`
          );

          if (currentChainId !== CONFIG.CHAIN_ID) {
            console.log(
              "  ⚠️  Wrong network detected, attempting to switch..."
            );
            ...
            try {
              ...
              if (switchError.code === 4001) {
                ...
                updateUIWithMessage(
                  `Please switch to ${CONFIG.NETWORK_NAME} manually`
                );
...
async function handleWalletConnected(account) {
  try {
    ...
    console.log(
      "getSigner() returned - is promise?",
      signerOrPromise instanceof Promise
    );
    ...
    if (network.chainId !== CONFIG.CHAIN_ID) {
      console.warn(
        `Wrong chain: ${network.chainId}, expected: ${CONFIG.CHAIN_ID}`
      );
      connectionState = ConnectionState.WRONG_NETWORK;
      updateUIForWrongChain();
      return;
...
async function payAndPlayGame() {
  ...
  if (!currentAccount) {
    ...
    await showModal(
      "Please connect your wallet to pay 100k m00nad and play.",
  ...
  if (!ethersProvider || !ethersSigner) {
    ...
    await showModal(
      "Wallet not properly connected. Please reconnect.",
      "Connection Error"
    );
    return false;
  }
  ...
  try {
    ...
    const tokenAddr = await getGameTokenAddress();
    const balance = await getPlayerBalance(currentAccount, tokenAddr);
    ...
    const allowance = await getPlayerAllowance(currentAccount, tokenAddr);
    if (allowance.lt(entryFeeInWei)) {
      const approved = await approveToken(entryFeeInWei, tokenAddr);
      if (!approved) {
...
async function submitScore(score, landed) {
  ...
  if (landed !== 0 && landed !== 1) {
    await showModal(
      "Invalid landing status. Please try again.",
      "Invalid Status"
    );
    return false;
  }
 ...
 */
async function getPlayerBalance(address, tokenAddress) {
  try {
    if (!address) {
    ...
    const tokenContract = new ethers.Contract(
      tokenAddress || gameTokenAddress || CONFIG.M00NAD_TOKEN,
      getERC20ABI(),
      readProvider
    );
    ...
    console.log(
      "Balance:",
      ethers.utils.formatUnits(balance, CONFIG.M00NAD_DECIMALS)
    );
 ...
 */
async function getPlayerAllowance(address, tokenAddress) {
...
async function getGameTokenAddress() {
  try {
    if (gameTokenAddress) return gameTokenAddress;
    if (!ethersProvider) return CONFIG.M00NAD_TOKEN;
    const contract = new ethers.Contract(
      CONFIG.MOONLANDER_CONTRACT,
      window.MOONLANDER_ABI || [],
      ethersProvider
    );
    const addr = await contract.m00nadToken();
    if (addr) {
      gameTokenAddress = addr;
      return addr;
    }
    return CONFIG.M00NAD_TOKEN;
  } catch (e) {
    console.warn("Could not read game token address:", e);
    return CONFIG.M00NAD_TOKEN;
  }
}
...
async function listenToScoreSubmittedEvents(callback) {
  try {
    ...
    moonlanderContract.on(
      "ScoreSubmitted",
      (player, score, landed, timestamp, event) => {
        console.log("ScoreSubmitted event:", {
          player,
...
function updateUIForConnectedWallet(address) {
  ...
  if (menuConnectBtn && menuWalletStatus) {
    menuWalletStatus.textContent = `${address.slice(0, 6)}...${address.slice(
      -4
    )}`;
  ...
  if (hudConnectBtn && hudWalletInfo) {
    ...
    if (hudWalletAddress) {
      hudWalletAddress.textContent = `${address.slice(0, 6)}...${address.slice(
        -4
      )}`;
    }
  }
...
function updateUIForWrongChain() {
  updateUIWithMessage(
    `Wrong network. Please switch to Monad Mainnet (Chain ID: ${CONFIG.CHAIN_ID})`
  );
